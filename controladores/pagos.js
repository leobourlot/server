...
//                 notification_url: `${process.env.BASE_URL}/api/v1/pagos/webHook?idOrganizador=${idOrganizador}`,
...
const crearOrden = async (req, res) => {
    try {
        const { idTorneo, idOrganizador } = req.body;
        if (!idTorneo || !idOrganizador) {
            return res.status(400).json({ message: "Falta el ID del torneo o del organizador" });
        }

        // Token del vendedor/organizador para crear la preferencia
        const vendedorToken = await obtenerToken(idOrganizador);
        const client = new MercadoPagoConfig({ accessToken: vendedorToken, options: { timeout: 5000 } });
        const preferenceApi = new Preference(client);
        ...
        const precio = parseInt(torneo[0].costoInscripcion, 10);

        // Crear preferencia con URL de webhook que incluye idOrganizador
        const resultado = await preferenceApi.create({
            body: {
                items: [
                    {
        ...
        return res.status(500).json({ message: "Algo salió mal al crear la preferencia" });
    }
};
...
const recibeWebHook = async (req, res) => {
    try {
        const payload = req.body;
        const paymentId = payload.data?.id;
        const idOrganizador = req.query.idOrganizador;

        if (!paymentId || !idOrganizador) {
            return res.status(400).send("Requiere paymentId y idOrganizador");
        }

        // Obtener token del organizador para consultar el pago
        const vendedorToken = await obtenerToken(idOrganizador);
        const client = new MercadoPagoConfig({ accessToken: vendedorToken, options: { timeout: 5000 } });
        const paymentApi = new Payment(client);
...
// Endpoint para consultar estado de pago (puede usar API key general)
const estadoPago = async (req, res) => {
    try {
        const paymentId = req.params.id;
        const apiKey = process.env.MERCADOPAGO_API_KEY;
        ...
        console.error('Error obteniendo estado del pago:', error);
        return res.status(500).json({ message: "Error al obtener el estado del pago" });
    }
};
