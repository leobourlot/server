...
nueva = async (req, res) => {
    ...
    const { titulo, descripcion, urlImagen, descripcionImagen, contenido } = req.body;
    ...
    if (!titulo || !descripcion || !urlImagen || !descripcionImagen || !contenido) {
...
modificar = async (req, res) => {
    ...
    if (!idNoticia || !titulo || !descripcion || !urlImagen || !descripcionImagen || !contenido) {
        res.status(404).json({ estado: 'FALLO', msj: 'faltan datos requeridos' });
    } else {
        const dato = {
            idNoticia: idNoticia,
